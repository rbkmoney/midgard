...
BEGIN
    ...
    ordered_clearing_trx_cte as (
          SELECT clearing_id, transaction_id, trx_type, invoice_id, payment_id, refund_id, trx_version,
                 row_number() over(partition BY clearing_id ORDER BY trx_type, transaction_id) as row_num
          FROM   (
                 SELECT clearing_id, transaction_id, invoice_id, payment_id, NULL as refund_id, trx_version, trx_type
                 FROM clearing_trx_cte
                   UNION ALL
                 SELECT clearing_id, transaction_id, invoice_id, payment_id, refund_id, trx_version, trx_type
                 FROM clearing_refund_trx_cte
                   UNION ALL
                 SELECT src_clearing_id, transaction_id, invoice_id, payment_id, refund_id, trx_version, transaction_type
    ...
    INSERT INTO midgard.clearing_event_transaction_info(clearing_id, transaction_id, invoice_id, payment_id, refund_id,
                                                        trx_version, transaction_type, row_number)
    SELECT clearing_id, transaction_id, invoice_id, payment_id, refund_id, trx_version, trx_type, row_num
