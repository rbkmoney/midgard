...
BEGIN
  ...
  failed_clr as (
    ...
    ordered_clearing_trx_cte as (
        SELECT clearing_id, transaction_id, trx_type, invoice_id, payment_id,
        ...
        FROM   (
               ...
               FROM clearing_refund_trx_cte
                 UNION ALL
               SELECT src_clearing_id, transaction_id, invoice_id, payment_id, transaction_type
               FROM midgard.clearing_event_transaction_info ti
               WHERE ti.clearing_id in (SELECT id FROM failed_clr)
        ) cte
    )

    INSERT INTO midgard.clearing_event_transaction_info(clearing_id, transaction_id, invoice_id, payment_id,
                                                        transaction_type, row_number)
    SELECT clearing_id, transaction_id, invoice_id, payment_id, trx_type, row_num
