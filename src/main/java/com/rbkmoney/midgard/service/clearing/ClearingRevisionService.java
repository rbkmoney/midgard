...
import static com.rbkmoney.midgard.domain.enums.ClearingEventStatus.ADAPTER_FAULT;
import static com.rbkmoney.midgard.domain.enums.ClearingEventStatus.CREATED;
import static com.rbkmoney.midgard.domain.enums.ClearingEventStatus.EXECUTE;
    ...
    private void processAdapterFaultClearingEvents() {
        ...
        List<ClearingEventInfo> adapterFaultEvents =
                eventInfoDao.getAllClearingEventsByStatus(ADAPTER_FAULT).stream()
                        .filter(event ->
                                event.getDate().plusHours(retriesHourCount)
